...
def preprocess(min_date:str = '2009-01-01', max_date:str = '2015-01-01') -> None:
    ...
    return X_processed,y, data_query # Me, Marine, I added this line of code here
...
def train(
    ...
    X_processed, y, data_query = preprocess() # Me, Marine, from line of code added in previous function

    data_cleaned = pd.DataFrame(X_processed)\
                                .merge(y,
                                    left_index=True,
                                    right_index=True)
    ...
    train_length = int(len(data_cleaned) * (1 - split_ratio))

    data_train = data_cleaned.iloc[:train_length, :].sample(frac=1)
    data_val = data_cleaned.iloc[train_length:, :].sample(frac=1)

    X_train_processed = data_train.drop("fare_amount", axis=1)
    y_train = data_train[["fare_amount"]]

    X_val_processed = data_val.drop("fare_amount", axis=1)
...
def evaluate(
    ...
    X_processed,y, data_processed = preprocess()
